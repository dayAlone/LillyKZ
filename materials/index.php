<?
	...
	if($_REQUEST['ELEMENT_CODE']) {
		...
		$rsSect = CIBlockSection::GetList(array(),array('CODE'=>$_REQUEST['ELEMENT_CODE'], 'IBLOCK_ID'=>6));
		$arSect = $rsSect->Fetch();
	...
	if($APPLICATION->GetCurDir()=='/materials/dysfunction/') {
		$APPLICATION->SetPageProperty('mobile_title', '<strong>Еркектердің әңгімесі</strong><span>Эректильді дисфункция туралы</span>');
		...
		$APPLICATION->SetPageProperty('mobile_title', '<strong>Еркектердің әңгімесі</strong><span>Қуықалды без аденомасы туралы</span>');
		...
		$APPLICATION->SetPageProperty('mobile_title', '<strong class="bottom">Еркектердің әңгімесі</strong>');
