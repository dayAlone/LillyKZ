<?	
	...
	function item_promo($item, $size, &$i = false, $blank = false, $url = "materials") {
      ...
      echo '<a href="'.(strstr($APPLICATION->GetCurDir(),'/ru/')?"/ru":"").(strlen($item['PROPERTIES']['LINK']['VALUE'])>0?$item['PROPERTIES']['LINK']['VALUE']:"/".$url."/".$item['CODE']."/").'" '.($blank?"target='_blank'":"").' class="block '.$size.' '.($i==0?"first":"").'" style="background-image:url('.(file_exists($_SERVER['DOCUMENT_ROOT'].$item['PREVIEW_PICTURE']['SRC'])?$item['PREVIEW_PICTURE']['SRC']:"http://ochenprosto.ru".$item['PREVIEW_PICTURE']['SRC']).')">
